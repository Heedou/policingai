...
from laworder.pipeline import LaworderPipeline, true_or_false, classification_scores, bool_type_transform
import logging
...
if __name__ == '__main__':
    ...
    logging.info(f"STEP [2] : Preparing the Laworder Pipeline to be evaluated >>>>> {conf.finetune.llm_backbone}")
    pipeline        = LaworderPipeline(conf, logging)
    ...
    logging.info(f"STEP [3] : Running Inference on the Laworder Pipeline >>>>> {conf.finetune.llm_backbone}")
    START_FLAG  = False
